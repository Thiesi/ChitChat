...
- at least one public room containing normal, `/me`, edited, deleted, and attachment messages;
- one direct-message conversation with an attachment and unread message;
- at least one privacy notification;
...
6. Navigate several messages in browse mode. Confirm author, time, edited/deleted state, message text, attachment name, metadata, and download control are understandable without relying on visual position.
7. Send a multiline message and operate the attachment picker entirely from the keyboard.
8. Open Direct messages. Confirm the privacy disclosure is encountered before or alongside the messaging interface, conversation controls are named, unread state is understandable, and incoming messages do not cause a focus jump.
9. Open Account and Privacy notifications. Confirm headings, details/summary controls, checkboxes, status messages, notification read state, and destructive-action wording are announced correctly.
10. As the Super-Administrator, open Administration, privileged step-up, room creation, direct-message inspection, and revision review. Confirm dialogs announce their title, initial focus is sensible, focus remains contained while open, Escape or Cancel closes them, and focus returns to the invoking control.
...
- Operate authentication tabs, room selection, message actions, attachment controls, details elements, notification controls, and administration navigation.
...
1. At 200% zoom on a 1280 CSS-pixel-wide desktop viewport, exercise authentication, chat, direct messages, Account, Privacy notifications, and Administration.
