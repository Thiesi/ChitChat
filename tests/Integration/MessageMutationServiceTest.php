...
final class MessageMutationServiceTest extends DatabaseTestCase
{
    ...
    public function testRoomAuthorEditAndModeratorDeleteCreateImmutableRevisions(): void
    {
        ...
        $this->pdo->exec('TRUNCATE realtime_events, audit_log RESTART IDENTITY');
        ...
        self::assertTrue($edited['can_delete']);
        self::assertSame('Edited room message', $messages->history($author, $room->id)[0]['body']);

        $revision = $this->pdo->query(
            'SELECT action, actor_user_id, body_before, body_after FROM room_message_revisions ORDER BY id',
        )->fetch();
        self::assertIsArray($revision);
        self::assertSame('edit', $revision['action']);
        self::assertSame($author->id, (int) $revision['actor_user_id']);
        self::assertSame('Original room message', $revision['body_before']);
        self::assertSame('Edited room message', $revision['body_after']);
        self::assertSame(
        ...
        self::assertSame(
            ['room_message'],
            array_map(static fn ($event): string => $event->type, (new EventRepository($this->pdo))->visibleAfter($author, 0)),
        );

        try {
            $mutations->edit($other, $message['id'], 'Unauthorized edit', '127.0.0.3');
            self::fail('Expected non-author room edit rejection.');
        } catch (ApiException $exception) {
            self::assertSame('message_author_required', $exception->errorCode);
        }

        $messages->delete($root, $message['id'], '127.0.0.1');
        $revisions = $this->pdo->query(
            'SELECT action, actor_user_id, body_before, body_after FROM room_message_revisions ORDER BY id',
        )->fetchAll();
        self::assertCount(2, $revisions);
        self::assertSame('delete', $revisions[1]['action']);
        self::assertSame($root->id, (int) $revisions[1]['actor_user_id']);
        self::assertSame('Edited room message', $revisions[1]['body_before']);
        self::assertNull($revisions[1]['body_after']);
    }

    public function testRoomAuthorDeletionRevokesAttachmentAndRetainsFileUntilCleanup(): void
    {
        $auth = new AuthService($this->pdo, $this->config);
        $root = $auth->register('Root', 'a very secure password', '127.0.0.1');
        $author = $auth->register('Author', 'another secure password', '127.0.0.2');
        ...
        $service = new AttachmentService($this->pdo, $config);
        ...
        $path = $service->authorizeDownload($author, $attachmentId)['path'];

        $state = (new RoomMessageMutationService($this->pdo))->deleteOwn(
            $author,
            $message['id'],
            '127.0.0.2',
        );
        self::assertTrue($state['deleted']);
        self::assertSame('author', $state['deletion_kind']);
        self::assertFileExists($path);
        try {
            $service->authorizeDownload($author, $attachmentId);
            self::fail('Expected deleted room attachment access rejection.');
        } catch (ApiException $exception) {
    ...
    public function testDirectMessageEditRespectsBlocksWhileDeletionRemainsAvailable(): void
    {
        $auth = new AuthService($this->pdo, $this->config);
        ...
        $revision = $this->pdo->query(
            'SELECT action, actor_user_id, body_before, body_after FROM direct_message_revisions ORDER BY id',
        )->fetch();
        self::assertIsArray($revision);
        self::assertSame('edit', $revision['action']);
        self::assertSame($alice->id, (int) $revision['actor_user_id']);
        self::assertSame('Original private message', $revision['body_before']);
        self::assertSame('Edited private message', $revision['body_after']);

        try {
            $mutations->edit($bob, $message['id'], 'Recipient edit', '127.0.0.2');
            self::fail('Expected recipient direct-message edit rejection.');
        } catch (ApiException $exception) {
            self::assertSame('message_author_required', $exception->errorCode);
        }
        ...
        try {
            ...
            self::fail('Expected blocked direct-message edit rejection.');
        } catch (ApiException $exception) {
        ...
        self::assertSame(
            ...
            array_map(static fn ($event): string => $event->type, (new EventRepository($this->pdo))->visibleAfter($alice, 0)),
        );
    }

    public function testDeletedDirectAttachmentIsRevokedThenRemovedByConfiguredCleanup(): void
    {
        $auth = new AuthService($this->pdo, $this->config);
        $alice = $auth->register('Alice', 'a very secure password', '127.0.0.1');
        $bob = $auth->register('Bob', 'another secure password', '127.0.0.2');
        ...
        try {
            ...
            self::fail('Expected deleted direct attachment access rejection.');
        } catch (ApiException $exception) {
